<?php namespace Tranthihoaitrang\Classmy\Models;

use Foostart\Category\Library\Models\FooModel;
use Illuminate\Database\Eloquent\Model;
use Foostart\Comment\Models\Comment;

class Classmy extends FooModel {
    ...
    public function setConfigs() {
        ...
        $this->fields = array_merge($this->fields, [
            ...
            'Classmy_overview' => [
                'name' => 'Classmy_overview',
                'type' => 'Text',
            ],
            'Classmy_description' => [
                'name' => 'Classmy_description',
                'type' => 'Text',
            ],
            'Classmy_image' => [
                'name' => 'Classmy_image',
            ...
            foreach($params as $column => $value)
            {
                if($this->isValidValue($value))
                {
                    switch($column)
                    {
                        ...
                        case '_id':
                            if (!empty($value)) {
                                $elo = $elo->where($this->table . '.Classmy_id', '!=', $value);
                            }
                            break;
                        case 'status':
                            if (!empty($value)) {
                        ...
                        case 'keyword':
                            if (!empty($value)) {
                                $elo = $elo->where(function($elo) use ($value) {
                                    $elo->where($this->table . '.Classmy_name', 'LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.Classmy_description','LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.Classmy_overview','LIKE', "%{$value}%");
                                });
                            }
                            break;
                        default:
