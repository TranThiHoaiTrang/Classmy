<?php namespace Tranthihoaitrang\Classmy\Controllers\Admin;
...
use Tranthihoaitrang\Classmy\Validators\ClassValidator;
...
class ClassAdminController extends FooController {
    ...
    public function __construct(Request $request) {
        ...
        //models
        $this->obj_item = new Classmy(array('perPage' => 10));
        ...
        //set language files
        $this->plang_admin = 'Classmy-admin';
        $this->plang_front = 'Classmy-front';

        //package name
        $this->package_name = 'Classmy';
        $this->package_base_name = 'Classmy';

        //root routers
        $this->root_router = 'Classmy';
        ...
        $this->category_ref_name = 'admin/Classmy';
        ...
        $this->breadcrumb_2['label'] = 'Classmy';
    ...
    public function index(Request $request) {
        ...
        if ($is_admin ) {
            ...
            return redirect()->route('Classmy.list', ['user_id' => $user['user_id']]);
    ...
    public function config(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/Classmy.php'));
        // $package_path = realpath(base_path('vendor/foostart/package-classes'));
        $package_path = realpath(base_path('vendor/Tranthihoaitrang/Classmy'));
        ...
        if ($request->isMethod('Classmy') && $is_valid_request) {

            //create backup of current config
            file_put_contents($config_bakup.'/Classmy-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        ...
        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/Classmy-admin.php'));
        ...
        if ($request->isMethod('Classmy') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                file_put_contents($lang_bakup.'/'.$key.'/Classmy-admin-'.date('YmdHis',time()).'.php', $content);
            }
